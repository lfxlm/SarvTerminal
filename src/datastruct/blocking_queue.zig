...
///     locks and bounds checks and do a one-time drain.
///
/// This queue supports multiple producers and a single consumer (MPSC). All
/// mutations are guarded by a mutex. A lock-free fast path was tried here
/// before but the queues it backs (e.g. the app mailbox shared across every
/// surface and renderer thread) have multiple producers, so the unsynchronized
/// write position caused messages to be overwritten and their heap buffers
/// leaked.
